...
SELECT name, IF( monthlymaintenance >100,  'expensive',  'cheap' ) AS monthlymaintenance
FROM Facilities
...
SELECT DISTINCT CONCAT( f.name,  ':: ', m.surname,  ', ', m.firstname ) AS Output
...
SELECT CONCAT (f.name,':',m.surname,',', m.firstname) AS Information, 
CASE WHEN m.memid=0
